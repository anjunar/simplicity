...
export class Parser {
    ...
    import(state, stack, walk, init, topLevel) {
        ...
        while (token.type === "comma" || token.type === "import") {
            token = state.next();
    ...
    class(state, stack, walk, init, topLevel) {
        ...
        if (token.type !== "leftCurlyBracket" && token.type !== "extends") {
            node.id = walk([...init, {...node, position : "id"}]);
        }

        token = state.current({expected: ["leftCurlyBracket", "extends"]});
    ...
    yield(state, stack, walk, init, topLevel) {
        ...
        if (token.type !== "rightCurlyBracket" && token.type !== "semicolon" && token.type !== "rightRoundBracket" && token.type !== "arrow")  {
            node.argument = walk([...init, {...node, position : "argument"}]);
        }
    ...
    for(state, stack, walk, init, topLevel) {
        ...
        if (token.type === "semicolon") {
            ...
            if (token.type !== "semicolon" && token.type !== "rightRoundBracket") {
    ...
    switch(state, stack, walk, init, topLevel) {
        ...
        while (token.type !== "rightCurlyBracket") {
            ...
            while (token.type !== "case" && token.type !== "default" && token.type !== "rightCurlyBracket") {
        ...
        if (this.isObjectPattern(init)) {
            ...
            while (token.type !== "rightCurlyBracket") {
                ...
                if (token.value === "get" || token.value === "set") {
                    ...
                    if (token.type === "colon" || token.type === "left") {
                        token = state.previous({keyWords : false});
                    }
                }
                ...
                } else if (token.type === "comma" || token.type === "rightCurlyBracket") {
                    node.properties.push({
                        type: "Property",
                        key: key,
                        value: key,
...
export function parser(expression, parsers = [new TopLevelParser(), new Parser()]) {
    let iterator = tokenizer(expression);
    ...
    function walk(init, options) {
        ...
        if (token) {
            ...
            throw new Error(`State not available : ${token.type} at position "${iterator.capture()}"`)
        }
    ...
    let state = {
        capture() {
            return iterator.capture();
        },
        previous(options = {keyWords: true}) {
            if (!Reflect.has(options, "keyWords")) {
                options.keyWords = true;
            }
            return iterator.previous(options).value;
        },
        next(options = {keyWords: true}) {
            if (!Reflect.has(options, "keyWords")) {
                options.keyWords = true;
            }
            return iterator.next(options).value
        },
        current() {
